# parquet_to_postgres
![Screenshot of a project interface](etl_parquet_postgres.jpg)
# *Overview*
...
# *Prerequisites*
To follow along this project need to available below requirement on system:
...
2. postgres connection
   ...
   ```
3. [E]xtract -- read parquet & convert into pandas data frame
   ```python3
   df = pd.read_parquet('CO2 Emission Country.parquet')
   ```
4. [T]ransform -- remove uncessary string & conver to numeric
   ```python3
   ...
   ```
5. [L]oad -- load pandas to postgres
   ```python3
   ...
   ```
6. PostgreSQL result
    ![PostgreSQL result](parquet_to_postgres.png)
